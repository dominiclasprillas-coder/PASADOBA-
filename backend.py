...
def ui_get_subjects():
    ...
    return jsonify({'subjects': database.list_ui_subjects(user_id)})
...
def ui_delete_subject(sid):
    ...
    database.delete_ui_subject(sid, user_id)
...
def network_info():
    try:
        ...
        local_ip = "127.0.0.1"
    return jsonify({'local_ip': local_ip, 'share_url': f'http://{local_ip}:5000'})

# These endpoints are deprecated — use /api/ui/subjects instead
# Kept for backwards compatibility but won't work without user_id support
...
def create_subject():
    ...
    if not name:
        return jsonify({'success': False, 'error': 'Name required'}), 400
    if database.subject_name_exists(name, user_id):
...
def get_subject(sid):
    user_id = request.headers.get('X-User-Id', 'default')
    subject = database.get_subject(sid, user_id)
    if not subject:
        return jsonify({'error': 'Subject not found'}), 404
    grade = calculate_subject_grade(subject)
    subject['current_grade'] = grade
...
def update_subject(sid):
    user_id = request.headers.get('X-User-Id', 'default')
    if not database.subject_exists(sid, user_id):
        return jsonify({'success': False, 'error': 'Subject not found'}), 404
    data = request.json or {}
    database.update_subject(sid, user_id, data)
...
def delete_subject(sid):
    ...
    if not subject:
        return jsonify({'error': 'Subject not found'}), 404
    database.delete_subject(sid, user_id)
...
def main():
    ...
    print(f"  Share:  http://{local_ip}:5000")
