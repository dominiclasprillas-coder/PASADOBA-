...
def subject_exists(subject_id, user_id):
    with _connect() as conn:
        row = conn.execute(
...
def subject_name_exists(name, user_id):
    with _connect() as conn:
        row = conn.execute(
            "SELECT 1 FROM subjects WHERE lower(name) = lower(?) AND user_id = ?",
            (name, user_id)
        ).fetchone()
        return row is not None

def create_subject(name, strategy, base_value, target_grade, user_id):
    subject_id = str(uuid.uuid4())[:8]
    base_value = base_value if base_value is not None else 0
    with _connect() as conn:
        conn.execute(
            "INSERT INTO subjects (id, user_id, name, strategy, base_value, target_grade) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (subject_id, user_id, name, strategy or 'raw', base_value, target_grade)
        )
    return subject_id

def update_subject(subject_id, user_id, data):
    fields = {}
    if 'name' in data:
    ...
    values = list(fields.values()) + [subject_id, user_id]
    with _connect() as conn:
        conn.execute(
            f"UPDATE subjects SET {assignments} WHERE id = ? AND user_id = ?",
            values
        )
...
def list_ui_subjects(user_id):
    with _connect() as conn:
        rows = conn.execute(
            "SELECT data FROM ui_subjects WHERE user_id = ?",
            (user_id,)
        ).fetchall()
        return [json.loads(row["data"]) for row in rows]

def ui_subject_exists(subject_id, user_id):
    with _connect() as conn:
        row = conn.execute(
            "SELECT 1 FROM ui_subjects WHERE id = ? AND user_id = ?",
            (subject_id, user_id)
        ).fetchone()
        return row is not None

def create_ui_subject(subject_id, subject, user_id):
    with _connect() as conn:
        conn.execute(
            "INSERT INTO ui_subjects (id, user_id, data) VALUES (?, ?, ?)",
            (subject_id, user_id, json.dumps(subject))
        )

def update_ui_subject(subject_id, subject, user_id):
    with _connect() as conn:
        conn.execute(
            "UPDATE ui_subjects SET data = ? WHERE id = ? AND user_id = ?",
            (json.dumps(subject), subject_id, user_id)
        )

def delete_ui_subject(subject_id, user_id):
    with _connect() as conn:
        conn.execute(
            "DELETE FROM ui_subjects WHERE id = ? AND user_id = ?",
            (subject_id, user_id)
        )
