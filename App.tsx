...
async function api<T>(path: string, options: RequestInit = {}, userId?: string): Promise<T> {
...
export default function App() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('pasadoba_user_id')
    if (stored) {
      setUserId(stored)
    }
  }, [])
  ...
  useEffect(() => {
    const loadSubjects = async () => {
      ...
      try {
        const data = await api<{ subjects: StoredSubject[] }>("/api/ui/subjects", {}, userId)
  ...
  const deleteSubject = async (id: string) => {
    if (!userId) return
    try {
      await api(`/api/ui/subjects/${id}`, { method: "DELETE" }, userId)
        ...
        <div className="w-full max-w-md">
          <BluePanel>
            <div className="text-center mb-8">
              <h1 className="text-3xl font-black text-white mb-2">PasadoBa?</h1>
              <p className="text-white/70 text-sm">Grade Tracker</p>
            </div>
            
            <div className="space-y-4">
              <p className="text-white/80 text-sm text-center">
                Enter your name or student ID to get started
              </p>
              <input
                type="text"
                value={userInput}
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter' && userInput.trim()) {
                    localStorage.setItem('pasadoba_user_id', userInput.trim())
                    setUserId(userInput.trim())
                  }
                }}
                className="w-full px-4 py-2.5 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:border-white/40 focus:ring-2 focus:ring-white/10 transition-all"
              />
              <button
                onClick={() => {
                  if (userInput.trim()) {
                ...
                className="w-full px-4 py-2.5 bg-white text-primary font-bold rounded-xl hover:bg-white/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                Continue
              </button>
            </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-foreground/60 font-medium">{userId}</span>
              <button
                onClick={() => {
                  localStorage.removeItem('pasadoba_user_id')
                  setUserId(null)
                  setSubjects([])
                }}
